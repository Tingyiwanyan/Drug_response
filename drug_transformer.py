...
class dotproductattention(tf.keras.layers.Layer):  #@save
	...
	def call(self, queries, keys, values, relative_encoding_lookup=None, edge_type_embedding=None,**kwargs):
		...
		if relative_encoding_lookup == None:
			...
			scores_ = tf.matmul(queries, keys, transpose_b=True)*0.25
			#print("scores_ shape")
			#print(scores_.shape)
			...
			scores_position = tf.reduce_sum(tf.multiply(queries_, relative_encoding_lookup), axis=-1)*0.25
			scores_edge_embedding = tf.reduce_sum(tf.multiply(queries_, edge_type_embedding), axis=-1)*0.5
...
class attention_embedding(tf.keras.layers.Layer):
	...
	def call(self, att_weights, input_value,relative_encoding_lookup=None, edge_type_embedding=None,**kwargs):
		...
		if relative_encoding_lookup == None:
			...
			value_ = value_*0.25
			relative_encoding_lookup = relative_encoding_lookup*0.25
			edge_type_embedding = edge_type_embedding*0.5
